...
-- Update the trigger function to use proper expertise mapping
CREATE OR REPLACE FUNCTION add_group_creator_as_owner()
RETURNS TRIGGER AS $$
DECLARE
    user_expertise text;
BEGIN
    -- Map degree level to expertise level
    SELECT CASE 
        WHEN degree_level = 'undergraduate' THEN 'Intermediate'
        WHEN degree_level = 'masters' THEN 'Advanced'
        WHEN degree_level = 'phd' THEN 'Advanced'
        WHEN degree_level = 'postdoc' THEN 'Advanced'
        ELSE 'Intermediate'
    END INTO user_expertise
    FROM user_profiles 
    WHERE id = NEW.created_by;

    -- Default to 'Intermediate' if user not found
    IF user_expertise IS NULL THEN
