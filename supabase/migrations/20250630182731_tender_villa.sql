/*
  ...
  This migration fixes the RLS policies for study groups and group members,
  and adds demo groups if they don't already exist.

  1. Clean up existing policies safely
  2. Create new simplified non-recursive policies
  3. Add demo groups for testing
*/
...
BEGIN
    ...
    DROP POLICY IF EXISTS "Allow creating groups" ON study_groups;
    DROP POLICY IF EXISTS "Allow updating owned groups" ON study_groups;
    DROP POLICY IF EXISTS "Allow deleting owned groups" ON study_groups;
    ...
    DROP POLICY IF EXISTS "Allow joining groups" ON group_members;
    DROP POLICY IF EXISTS "Allow leaving groups" ON group_members;
    DROP POLICY IF EXISTS "Allow owners to manage members" ON group_members;
...
-- Study Groups Policies
CREATE POLICY "sg_select_public"
  ON study_groups
  FOR SELECT
  TO authenticated
  USING (is_private = false);

CREATE POLICY "sg_select_owned"
  ON study_groups
  FOR SELECT
  TO authenticated
  USING (created_by = auth.uid());

CREATE POLICY "sg_insert_own"
  ON study_groups
  FOR INSERT
  TO authenticated
  WITH CHECK (created_by = auth.uid());

CREATE POLICY "sg_update_owned"
  ON study_groups
  FOR UPDATE
  TO authenticated
  USING (created_by = auth.uid());

CREATE POLICY "sg_delete_owned"
  ON study_groups
  FOR DELETE
  TO authenticated
  USING (created_by = auth.uid());
...
CREATE POLICY "gm_select_public_groups"
  ON group_members
  FOR SELECT
  TO authenticated
  USING (
...
CREATE POLICY "gm_select_owned_groups"
  ON group_members
  FOR SELECT
  TO authenticated
  USING (
